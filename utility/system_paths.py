...
def get_current_host():
    ...
    # Remove temporary container ID attached to host name
    cluster_name_prefix_online = [s.hostname for s in get_known_systems() if s.description == 'CLUSTER-ONLINE'][0]
    cluster_name_prefix_local = [s.hostname for s in get_known_systems() if s.description == 'CLUSTER-LOCAL'][0]
    if cluster_name_prefix_online in current_host:
        current_host = cluster_name_prefix_online
    elif cluster_name_prefix_local in current_host:
        current_host = cluster_name_prefix_local
