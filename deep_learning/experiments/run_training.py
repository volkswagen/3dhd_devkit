...
def setup_config_and_logging(config_file):
    ...
    code_path = dirname(dirname(dirname(abspath(__file__))))
    target_path = join(current_log_dir, '3dhd_devkit')
    folders = ['dataset', 'dataset_api', 'deep_learning', 'scripts', 'torchplus', 'utility', 'visualization']
#    for folder in folders:
 #       target = Path(join(target_path, folder))
  #      src = Path(join(code_path, folder))
